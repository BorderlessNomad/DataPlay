...
func SetDefaults(res http.ResponseWriter, req *http.Request, prams martini.Params) string {
	...
	Database.DB.Exec("UPDATE priv_onlinedata SET Defaults = $1 WHERE GUID = $2 LIMIT 1", jsondata, prams["id"])
...
func GetDefaults(res http.ResponseWriter, req *http.Request, prams martini.Params) string {
	...
	rows := Database.DB.QueryRow("SELECT Defaults FROM priv_onlinedata WHERE GUID = $1 LIMIT 1", prams["id"])
	var output string
	rows.Scan(&output)
