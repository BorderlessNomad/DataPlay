...
func HandleLogin(res http.ResponseWriter, req *http.Request) {
	...
	rows, e := Database.DB.Query("SELECT `password` FROM priv_users where email = ? LIMIT 1", username)
	...
	if usrpassword != "" && bcrypt.CompareHashAndPassword([]byte(usrpassword), []byte(password)) == nil { // Check the password with bcrypt
		var uid int
		e := Database.DB.QueryRow("SELECT uid FROM priv_users where email = ? LIMIT 1", username).Scan(&uid)
		check(e)
		...
		var md5test int
		e := Database.DB.QueryRow("SELECT count(*) FROM priv_users where email = ? AND password = MD5( ? ) LIMIT 1", username, password).Scan(&md5test)
		...
		if e == nil {
			if md5test != 0 {
				...
				if e == nil {
					Database.DB.Exec("UPDATE `DataCon`.`priv_users` SET `password`= ? WHERE `email`=?", pwd, username)
...
func HandleRegister(res http.ResponseWriter, req *http.Request) string {
	...
	rows, e := Database.DB.Query("SELECT COUNT(*) FROM priv_users where email = ? LIMIT 1", username)
	check(e)
	rows.Next()
	...
	if doesusrexist == 0 {
		...
		r, e := Database.DB.Exec("INSERT INTO `DataCon`.`priv_users` (`email`, `password`) VALUES (?, ?);", username, pwd)
		if e != nil {
