...
func GenerateChartData(chartType string, guid string, names XYVal, charts *[]TableData, ind Index, reduce bool) {
	...
	} else if chartType == "column dated" && (names.Xtype == "float" || names.Xtype == "integer" || names.Xtype == "real") && IsDateYear(names.X) && names.Ytype == "" {
		y, _ := strconv.Atoi(names.Y)
		sql = fmt.Sprintf("SELECT * FROM %q WHERE EXTRACT(year FROM %q) = %d", guid, names.X, y)
	...
	} else if chartType == "column dated" { // single column pie chart x = type, y = count
		for rows.Next() {
			if names.Xtype == "date" && (names.Ytype == "float" || names.Ytype == "integer" || names.Ytype == "real") {
				rows.Scan(columnPointers...)

				// fmt.Println(names.Xtype, names.Ytype, columnNames)
