...
func GetLastVisited(uid int) ([]interface{}, *appError) {
	...
	if uid != 0 {
		...
		query := DB.Select("MAX (priv_tracking.id) id, priv_tracking.guid, (SELECT priv_index.title FROM priv_index WHERE priv_index.guid = priv_tracking.guid LIMIT 1) as title, MAX (priv_tracking.created) created")
		query = query.Joins("LEFT JOIN priv_index ON priv_index.guid = priv_tracking.guid")
		...
		// err = DB.Select("MAX (priv_tracking.id) id, priv_tracking.guid, (SELECT priv_index.title FROM priv_index WHERE priv_index.guid = priv_tracking.guid LIMIT 1) as title, MAX (priv_tracking.created) created").Joins("LEFT JOIN priv_index ON priv_index.guid = priv_tracking.guid").Where("title != ?", "").Where("priv_tracking.user = ?", uid).Group("priv_tracking.guid").Order("created DESC").Order("priv_tracking.guid DESC").Limit(5).Find(&results).Error
