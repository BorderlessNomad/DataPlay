...
func TermFrequency(terms []TermKey) ([]PoliticalActivity, error) {
	...
	cassandra, err := GetCassandraConnection("dp") // create connection to cassandra
	if err != nil {
		return nil, err
	}
	defer cassandra.Close()

	iter1 := cassandra.Query(`SELECT date, name FROM keyword WHERE date >= ? AND date < ? ALLOW FILTERING`, FromDate, Today).Iter()
	...
	iter2 := cassandra.Query(`SELECT date, name FROM entity WHERE date >= ? AND date < ? ALLOW FILTERING`, FromDate, Today).Iter()
