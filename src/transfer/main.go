...
func DataTransfer() {
	...
	for i = 0; i < 58; i++ {
		...
		var pic_index int
		var date time.Time
		var description, url, url2, title, name, pic_url, related_url string
		iter1 := session1.Query(`SELECT date, description, url, title FROM response WHERE date >= ? AND date < ? ALLOW FILTERING`, FromDate, ToDate).Iter()

		for iter1.Scan(&date, &description, &url, &title) {
			session2.Query(`INSERT INTO response2 (date, dummy, description, url, title) VALUES (?, ?, ?, ?, ?)`, date, 1, description, url, title).Exec()

			iter2 := session1.Query(`SELECT url, pic_index FROM image WHERE date = ? ALLOW FILTERING`, date).Iter()
			...
			err3 := iter2.Close()
			if err3 != nil {
				fmt.Println("ERROR LOOP image," i)
			} else {
				fmt.Println("SUCCESS LOOP image," i)
			}

			iter3 := session1.Query(`SELECT description, title, url FROM related WHERE date = ? ALLOW FILTERING`, date).Iter()
		...
		err3 := iter1.Close()
		if err3 != nil {
			fmt.Println("ERROR LOOP response," i)
		} else {
			fmt.Println("SUCCESS LOOP image," i)
		}
		...
		if err3 != nil {
			fmt.Println("ERROR LOOP keyword," i)
		} else {
		...
		err3 := iter5.Close()
		if err3 != nil {
			fmt.Println("ERROR LOOP entity," i)
		} else {
