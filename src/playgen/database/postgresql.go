...
func (self *PostgreSQL) Setup() {
	...
	flag.StringVar(&self.Host, "DBHost", "109.231.121.188", "Where to connect to the postgresql DB")
	flag.StringVar(&self.Port, "DBPort", "5432", "Where to connect to the postgresql DB")

	flag.StringVar(&self.Schema, "DBPostgreSQL", "dataplay", "The database name to use while connecting to the postgresql DB")
	flag.BoolVar(&self.Debug, "DBDebug", false, "Debug DB Queries")
}

func (self *PostgreSQL) ParseEnvironment() {
	databaseHost := "109.231.121.188"
	databasePort := "5432"
...
func (self *PostgreSQL) Connect() (err error) {
	...
	if err != nil {
		Logger.Println("[PostgreSQL] Error while connecting " + err.Error())
		return err
	}

	Logger.Println("[PostgreSQL] Connected! " + self.User + " @ " + self.Host + " : " + self.Port + " / " + self.Schema)
