	...
	.controller 'OverviewCtrl', ['$scope', '$routeParams', 'Overview', 'PatternMatcher', ($scope, $routeParams, Overview, PatternMatcher) ->
		...
		$scope.drawLineChart = (entry, fixedId, xScale, ordinals) ->
			...
			if ordinals?
				...
				#   $($scope.container).find("##{entry.type}-#{entry.x}-#{entry.y}").remove()
				# else
				#   chart.yAxis().ticks(3)

			console.log "drawLineChart CHART", chart
				...
				do (entry) =>
					...
					for dcChart in $scope.charts
						do (dcChart) =>
							...
							) and lastCharts.indexOf(dcChart.id)<0
								chartId = dcChart.id
								lastCharts.push dcChart.id
								return

					chart = switch chartId
						when 'rows' then $scope.drawRowsChart entry, fixedId
						when 'bars' then $scope.drawBarsChart entry, fixedId, xScale, m
						when 'pie' then $scope.drawPieChart entry, fixedId
						when 'bubbles' then $scope.drawBubblesChart entry, fixedId, xScale
					...
					plotChart = "line"
					if chartId in ['bars', 'pie', 'bubbles']
						plotChart = chartId
